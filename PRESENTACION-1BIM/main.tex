...
\textbf{RNF4.2} & \textbf{Escalabilidad:} El esquema relacional PostgreSQL y Prisma deben diseñarse bajo estándares que permitan una transición directa a APIs móviles. \\
\bottomrule
\end{tabular}
...
\textit{Nota: Los prototipos de interfaz se encuentran actualmente en fase de diseño y desarrollo activo.}

El prototipo de interfaz gráfica prioriza la simplicidad y el contraste visual para contrarrestar el analfabetismo digital de los pequeños minoristas:

\begin{enumerate}
    \item \textbf{Pantalla de Catálogo Mayorista (Vista Tendero):}
    \begin{itemize}
        \item \textbf{Estructura:} Encabezado superior persistente con el total acumulado de la compra y un indicador visual de barra de progreso que indica si ya se superó el monto mínimo (ej. ``Faltan \$15 para llegar al mínimo'').
        \item \textbf{Lista de Productos:} Grid de tarjetas anchas en formato vertical con fotos nítidas. Cada tarjeta posee un botón flotante verde grande para añadir y otro rojo para restar cantidades, con etiquetas de stock disponible en color verde brillante (stock alto) o naranja (stock bajo).
    \end{itemize}
    
    \item \textbf{Pantalla de Registro de Pedidos (Vista Vendedor Freelance):}
    \begin{itemize}
        \item \textbf{Estructura:} Menú lateral con accesos directos a ``Clientes'', ``Nuevo Pedido'', ``Comisiones'' e ``Historial''.
        \item \textbf{Formulario Dinámico:} Selector rápido para buscar el comercio local (auto-completado). El vendedor selecciona los productos recorriendo el catálogo optimizado y el sistema calcula la comisión proyectada en tiempo real antes de presionar ``Confirmar Orden''.
    \end{itemize}

    \item \textbf{Panel de Certificaciones (Vista Vendedor Freelance):}
    \begin{itemize}
        \item \textbf{Estructura:} Listado de cursos y exámenes disponibles organizados por empresa.
        \item \textbf{Flujo del Examen:} Interfaz tipo cuestionario con preguntas paso a paso para evitar la sobrecarga visual, mostrando un cronómetro flotante e indicaciones sencillas al finalizar que notifican el puntaje obtenido y el estado de habilitación de forma instantánea.
    \end{itemize}
\end{enumerate}
